...
export default function BookDetails() {
  ...
  const handleReadNow = () => {
    ...
    // For free books, add to reading list and navigate to first chapter
    if (book.isFree) {
      addToReadingList(book.id);
      if (book.sampleChapters.length > 0) {
        navigate(`/read/${book.id}/${book.sampleChapters[0].id}`);
      }
      return;
    }

    // For paid books, show payment modal
    setShowPayment(true);
  };
  ...
  return (
    <div className="max-w-6xl mx-auto">
      ...
      {showPayment && (
        <PaymentModal 
          plan={{ name: book.title, price: book.price, period: 'one-time' }}
